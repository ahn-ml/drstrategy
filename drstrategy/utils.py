...
# For visualizing reward
def draw_on_img_np(np_imgs, text_func, texts, fontsize=10):
    for t in range(len(np_imgs)):
        np_img = np_imgs[t]
        label = text_func(texts[t])
        img = Image.fromarray(np_img)
        pwd = os.path.dirname(os.path.abspath(__file__))
        font = ImageFont.truetype(os.path.join(pwd,"arial.ttf"), fontsize)
        draw = ImageDraw.Draw(img)
        draw.text((0,0), label, (1,1,0), font=font)
        img = np.array(img)
        np_imgs[t] = np.array(img)
    return np_imgs


def setup_wandb(cfg, workdir=None, load=False, id=None, sync_tensorboard=False):
    postfix = cfg.get("wandb_postfix", '')
    tags = cfg.get("tags", '').replace(' ', '').split(',')
    tags = [] if len(tags) == 1 and tags[0]=='' else tags
    tags.append(cfg.task)
    tags.append('non_episodic' if cfg.non_episodic else 'episodic')
    _se = 'SE' if cfg.agent.use_skilled_explore else 'LE' 
    server_name = os.getenv("HostName", "Unknown")+os.getenv("ServerNum",'?')
    exp_name = [_se, cfg.group, cfg.task, 'Seed'+str(cfg.seed), server_name]
    if postfix != '':
        exp_name.append(postfix)
    if workdir is not None:
        workdir_name = f'{workdir.parent.name}-{workdir.name.split("_")[0]}'
        exp_name.append(workdir_name)
    ...
    if not load:
        ...
        wandb_name = "-".join(wandb_name.split("-")[2:])  # only get the numeric part
        exp_name = f"{exp_name}-{wandb_name}"
        ...
        wandb_name = wandb.run.name
        wandb_name = "-".join(wandb_name.split("-")[2:])  # only get the numeric part
        exp_name = f"{exp_name}-{postfix}{wandb_name}{workdir_name}"
        wandb.run.name = exp_name
...
def print_dict_tensor_shape(dict_tensor):
    print(
        {k: v if not hasattr(v, "shape") else v.shape for k, v in dict_tensor.items()}
    )


def print_dict_tensor_dtype(dict_tensor):
    print(
        {k: v if not hasattr(v, "dtype") else v.dtype for k, v in dict_tensor.items()}
    )
