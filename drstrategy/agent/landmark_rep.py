...
class LandmarkModule(nn.Module):
  def __init__(self, config, landmark_dim, vq_ema=True, code_dim=16, code_resampling=True, resample_every=200):
    super().__init__()
    self.cfg = config
    ...
    self.vq_coef    = 0.05
    self.landmark_encoder = common.MLP(config.rssm.deter, (code_dim), **mlp_config)
    self.landmark_decoder = common.MLP(code_dim, (config.rssm.deter), **mlp_config)
    self.emb = NearestEmbedEMA(landmark_dim, code_dim) if self.vq_ema else NearestEmbed(landmark_dim, code_dim)
    self.all_params = list(self.emb.parameters()) + list(self.landmark_encoder.parameters()) + list(self.landmark_decoder.parameters())

    opt_config = { **self.cfg.actor_opt }

    self.latent_info = dict()
    self.landmark_opt = common.Optimizer('landmark', self.all_params, **opt_config, use_amp=self._use_amp)
  ...
  def update(self, seq,):
    ...
    with common.RequiresGrad(self):
      with torch.cuda.amp.autocast(enabled=self._use_amp):
        landmark_loss, metrics = self.landmark_loss(seq)
      metrics.update(self.landmark_opt(landmark_loss, self.all_params))
    self._updates += 1
    if self._updates % self.resample_every == 0:
      with torch.no_grad():
        z_e = self.landmark_encoder(seq['deter']).mean
        self.latent_info.update(self.emb.kmeans(z_e, update=self.code_resampling))
    metrics.update(self.latent_info)
    return metrics

  def landmark_loss(self, seq):
    z_e = self.landmark_encoder(seq['deter']).mean

    if self.vq_ema:
      emb, _ = self.emb(z_e, training=True)
      recon = self.landmark_decoder(emb).mean
      ...
      recon = self.landmark_decoder(z_q).mean
