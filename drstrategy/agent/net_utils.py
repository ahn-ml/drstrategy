...
class EnsembleRSSM(nn.Module):
  ...
  def get_deter_from_embed(self, embed, should_sample=True):
    # deterministic state is collected through the first observation and rollout with zero action
    state = self.initial(embed.shape[0])
    action = torch.zeros([embed.shape[0], self._action_dim], device=self.device)
    x = torch.cat([state['deter'], embed], -1)
    x = self._obs_out(x)
    x = self._act(x)
    stats = self._suff_stats_layer('_obs_dist', x)
    dist = self.get_dist(stats)
    stoch = dist.sample() if should_sample else None 
    state = {'stoch': stoch, 'deter': state['deter'], **stats}
    state = self.img_step(state, action, should_sample)
    return state['deter']
