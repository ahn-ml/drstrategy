...
class Logger(object):
    ...
    def log(self, key, value, step):
        assert key.startswith('train') or key.startswith('eval') or key.startswith('global_step')
    ...
    def log_global_step(self, step):
        self.log(f'global_step', step, step)
    ...
    def log_video(self, data, step):
        ...
        if self.use_wandb:
            for k, v in data.items():
                ...
                wandb.log({k: wandb.Video(v, fps=15, format="gif")}, step=step)
    
    def log_image(self, data, step):
        if self.use_wandb:
            for k, v in data.items():
                v = np.uint8(v.cpu() * 255)
                wandb.log({k: wandb.Image(v)}, step=step)
